...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((prev, curr) => curr.purchasePrice + prev, 0);
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((previous, current) => current['children'] ? current.children.length + previous : previous, 0);
...
const calculateAverage = (arr) => {
  // Solution code here...
  let acc = arr.reduce((previous, current) => {
    previous.count++;
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((previous, current) => isPrime(current) ? previous + 1 : previous, 0);
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce((previous, current) => current.stat.name === statName ? current : previous , null );
...
const extractChildren = (arr) => {
  // Solution code here...
  let namesWithA = arr.filter(el => el.name.toLowerCase().includes('a'));
  return namesWithA.reduce((previous, current) => current['children'] ? previous.push(...current.children) && previous: previous,[]);
};
