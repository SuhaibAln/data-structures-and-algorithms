...
const sortPeopleBetter = (arr) => {
  return arr.sort((a, b) => {
    if ([a.lastName, a.firstName, a.age] < [b.lastName, b.firstName, b.age]){
      return -1;
    }
    else if ([a.lastName, a.firstName, a.age] > [b.lastName, b.firstName, b.age]){
      return 1;
    }
    else {
      return 0;
...
const sortMeetingsByDay = (arr) => {
  ...
  return arr.sort((meeting1, meeting2) => days[meeting1.dayOfWeek] - days[meeting2.dayOfWeek]);
};
...
const sortSchedule = (arr) => {
  // Solution code here...
  let days = {'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3, 'Friday': 4};
  //no need to sort everything at once
  //like a spreadsheet, start with the last field; then move on. equal fields will keep their relative position.
  //sort by meeting lengths:
  let output = arr.sort((meeting1, meeting2) => (parseInt(meeting1.end) - parseInt(meeting1.start)) - (parseInt(meeting2.end) - parseInt(meeting2.start)));
  //sort by meeting start:
  output = output.sort((meeting1, meeting2) => parseInt(meeting1.start) - parseInt(meeting2.start));
  //sort by day of week:
  output = output.sort((meeting1, meeting2) => days[meeting1.dayOfWeek] - days[meeting2.dayOfWeek]);
  return output;
};
